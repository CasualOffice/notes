...
| [casual-note-calendar.md](./casual-note-calendar.md) | **Calendar & System-Calendar Sync** — the fifth surface: a local calendar unifying schedule + notes/tasks/reminders/meetings, with privacy-first two-way sync to the user's *own* system calendar (native / CalDAV / ICS, capability-tiered). *Supersedes the earlier "no calendar integration" v1 non-goal.* |
